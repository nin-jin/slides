...
![](https://raw.githubusercontent.com/nin-jin/slides/master/reactivity/reactivity-reactivity.svg)
...
- 🤹‍♂️ Only Necessary Calculations
- 🐵 Stable Behavior
- 🐘 Minimal Memory Footprint
- 💫 Constant Consistency of States

## Wish: 🤹 Only Necessary Calculations
...
## Wish: 💫 Constant Consistency of States
...
Наиболее продвинутые библиотеки, типа $mol_wire, делают глубокое сравнение нового и старого значения.
...
Как видите, `A` и `B` тут у нас отличаются, но свойство `bar` осталось таким как было. Это хорошо с точки зрения GC, ведь переиспользуем объекты, находящиеся в старом поколении сборщика мусора. А объект из молодого поколения был выборошен, что очень быстро.
...
К тому же, если объекты фактически идентичны, нам надо сначала их глубоко сравнить. И если они одинаковы, то вернуть старый, а несли нет - начать менять новый. Ну либо сразу начать менять новый, чтобы потом выяснить, что мы поменяли все его свойства, так что вернуть надо старый.

Короче, данный подход не самый шустрый и надёжный. Встретить его можно в библиотеке $mol_atom2, на которой работает фреймворк $mol, но мы планируем отказаться от него в пользу глубокого сравнения с кешированием.
...
В серии статей от Криса Гаррета можно ознакомиться с тем, как была переосмыслена реактивная модель во фреймворке Ember. Я оставил ссылку на наиболее релевантную из них.

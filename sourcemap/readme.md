...
В примере исходника используется имя `next`, но в рантайме такой переменной нет, ибо в сгенерированном коде переменная называется `pipe`. Однако, при наведении на `next`, браузер делает обратный маппинг и выводит уже значение именно переменной `pipe`.

...
> Но что если работа у нас не одна, а несколько, но поток-то один? Представьте, что вы гоните на своём Феррари и подъезжаете к железнодорожному переезду. Когда он свободен, вы можете проскочить его за долю секунды, но когда он занят километровым составом, вам приходится стоять и ждать несколько минут, пока он не проедет. 
...
const getConfig = fiberize( ()=> {
	...
	$mol_fiber_catch( error => ({ user : 'Anonymous' }) )

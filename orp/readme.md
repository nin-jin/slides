...
*Диаграмма зависимостей: отфильтрованнный список зависит от свойств каждого товара*

> На этой диаграмме вы видите зависимости между отфильтрованным списком товаров и данными самих товаров. Пока мы обновляем каждый товар по отдельности - всё хорошо, но стоит нам изменить сразу несколько состояний, то на каждое изменение у нас будет происходить полный набор вычислений вплоть до рендеринга. Поэтому-то и приходится вставлять везде всякие debounc-ы и trottl-инги, чтобы распространение обновлений происходило не сразу, а откладывалось на ближайшее время.
...
> Поэтому, прежде чем старая страница будет уничтожена, она будет обновлена. Это мало того, что медленно, так ещё и может приводить к ошибкам. Например, если в ссылке указан идентификатор, то показывается страница с подробностями по этому идентификатору. Когда же идентификатор пропадает из ссылки, то страница должна быть просто закрыта, но попытка её обновить приведёт лишь к ошибке, так как идентификатора уже нет.

# В порядке создания?

> [VueJS](http://vuejs.org) использует под капотом свою реализацию ОРП. Там обновление состояний происходит в порядке увеличения порядкового номера, задаваемого при создании реактивного свойства. Сначала это работает хорошо, но чем дольше работает ваше приложение тем в более случайном порядке происходит обновление состояний, порождая всё те же проблемы лишних пересчётов.
...
render() {
	...
	patch_node( node , {
		attrs : this.attrs() ,
		childNodes : this.sub_components().map( comp => comp.render() ) )
	} )

...
Чтож, сформулируем простейший и максимально неэффективный анализатор останова. Для начала возьмём некоторый конечный объём памяти и заметим, что число возможных состояний этой памяти тоже конечно. А так как работа исполнителя целиком и полностью определяется состоянием памяти, то он неизбежно придёт либо в терминиирующее состояние, либо в одно из состояний, в котором он уже был, и соответственно пойдёт по уже проложенному пути.

![](stop.png)

Получается, всё, что нам нужно делать - это исполнить алгоритм, на каждом шаге запоминая состояние памяти и сравнивая его со всеми предыдущими состояниями. Как только состояния совпадут - говорим, что алгоритм не останавливается.

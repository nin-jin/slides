...
> Здравствуйте, меня зовут Дмитрий Карловский и я.. безработный. Поэтому у меня есть много свободного времени для занятия музыкой, спортом, творчеством, языками и компьютерной наукой. Об последнем исследовании в области полуавтоматического разбиения долгих вычислений на небольшие кванты я вам сегодня и расскажу. Но сперва давайте обозначим проблемы, которые мы будем решать..
...
> А как было бы классно, если бы этот состав был разбит на 10 составов по 100 метров! Вы бы тогда не так уж сильно и задержались.
...
![(Де)сериализация](de-serialization.jpg)
...
![Очереди сообщений](async-only.png)
...
> Файбер просто подписывается на resolve обещания и перезапускается. Но вручную кидать и ловить обещания нет необходимости, ведь в комполект входят несколько полезных обёрток..
...
> Но к сожалению нативный `fetch` не поддерживает отмену запросов. Чтобы реализовать эту возможность мы можем воспользоваться `XMLHttpRequest`...
...
> Функция передаваемая в обёртку `async` вызывается лишь один раз и ей передаётся обёртка `back` в которую заворачиваются колбэки. Соответственно в колбэках этих нужно либо вернуть значение, либо бросить исключение. Каким бы нибыл результат работы колбэка - он станет и результатом файбера. Обратите внимание, что в конце мы возвращаем функцию, которая будет вызывана в случае преждевременного уничтожения файбера.

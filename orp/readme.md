...
> **Игрушек** у нас **много**, а начать продажи надо было ещё вчера.

> Мы можем загрузить все данные на клиентское устройство и **быстро, без сетевых задержек**, позволить пользователю просматривать каталог в реальном времени отслеживая изменения в нём.
...
> Если вы будете отображать все данные, что подготовили, то алгоритмическая сложность рендеринга будет пропорциональная объёму этих данных. 10 товаров рендерятся мгновенно, 1000 - с задержкой, а если 10000, то пользователь успеет сходить попить чайку.

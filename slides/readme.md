...
Давайте откроем капот и посмотрим, как устроено приложение, и чего ещё оно умеет, что не видно с первого взгляда.
...
Перед вами верхнеуровневое описание приложения на языке [view.tree](https://github.com/eigenmethod/mol/tree/master/view#viewtree). Давайте последовательно разберём каждый токен. Тут мы объявляем компонент `$hyoo_slides`, который будет расширять базовый компонент `$mol_view`. У этого компонента есть свойство `sub`. Всё, что возвращает это свойство, будет отрендерено внутри компонента. Поэтому мы переопределяем его, передавая в качестве значения массив из 3 элементов: `Loader` - компонент загрузчика , `Listener` - компонент вывода слайдов слушаетям и `Speaker` - компонент дополнительной панели докладчика.

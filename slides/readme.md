...
Но повторять эти кодовые фразы по десять раз подряд - это скучно, поэтому `$hyoo_slides` умеет анализировать заметки докладчика икогда вы произносите последнее слово, переключать слайд автоматически.

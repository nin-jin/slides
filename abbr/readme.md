...
Суть этого подхода заключается в ритуализации процесса разработки. То есть в некритическом безусловном выполнении определённых простых действий. Этот ритуал сделает ваш код красивым, содержащим минимум дефектов. Поддерживать его будет легко и просто. А разработка будет простой и быстрой. Так во всяком случае настоятельно убеждают нас проповедники TDD.
...
Тогда тесты покраснеют. А после того как мы откатим изменение, тесты статун зелёными.

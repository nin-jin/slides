...
> Решение в лоб - как только состояние изменилось - последовательно идём по зависимым состояниям и обновляем их и по мере их обновления обновляем и зависимые от них и так далее. Именно так работает по умолчанию OORP библиотека [KnockOut](http://knockoutjs.com) и наверно почти все FRP библиотеки.
...
![Ромб разрезанный на слои](diamond-slices.svg)
...
> А что если я скажу вам, что следующий код не смотря на свою синхронность может быть не только неблокирующим, но и грузить тексты с информацией о пользователе параллельно? 
...
Диаграмма взаимодействия пользователя и сервера через интерфейс приложения.

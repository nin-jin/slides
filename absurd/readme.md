...
Понятное дело, что анализатор на таком алгоритме не сможет вернуть ни правду, ни ложь, так как Санта сформулирован в форме абсурда. Так значит такого анализатора не существует?Да нет же. Такой анализатор должен просо бросить ошибку, так же как и для любого другого некорректного кода. В данном же случае текст ошибки должен быть такой: "алгоритм не корректен, так как его поведение зависит от результата работы анализатора".

То есть по сути, рассуждения Тьюринга говорят нам не о том, что анализатора быть не может, а то, что классической (бинарной) логики не достаточно для представления результата его работы - нужна как минимум троичная, в которой представимо в том числе и состояние сообщение об ошибке.

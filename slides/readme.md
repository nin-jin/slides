...
На представленном видео, вы видите квинтессенцияю того, куда катится современная индустрия веб разработки.
...
Давайте откроем капот и посмотрим, как устроено приложение, и что ещё оно умеет, чего не видно с первого взгляда.
...
Перед вами верхнеуровневое описание приложения на языке [view.tree](https://github.com/eigenmethod/mol/tree/master/view#viewtree). Давайте последовательно разберём каждый токен. Тут мы объявляем компонент `$hyoo_slides`, который расширяет базовый компонент `$mol_view`. У этого компонента есть свойство `sub`. Всё, что возвращает это свойство, будет отрендерено внутри компонента. Поэтому мы переопределяем его, передавая в качестве значения массив из двух элементов: `Listener` - компонент вывода слайдов слушаетям и `Speaker` - компонент дополнительной панели докладчика.
...
Тут может быть лубая, сколь угодно сложная логика, зависящая от самых разных условий. Но у нас логика будет простая:
...
role( next? : 'speaker' | 'listener' ) {
    return $mol_state_arg.value( 'role' , next ) || 'speaker'
}
```

...
Хаки позволяют не просто дословно транслировать однин язык в другой. С их помощью, можно экстрактировать из кода интересующую нас информацию. Например, у нас есть скрипт на некотором простом DSL, который что-то там выводит на английском языке..
...
И тут вы, скорее всего, задаётесь вопросом: "Дима, зачем ещё один велоспед? Горшочек, не вари! Хватит уже велосипедов!". Я бы и рад, но давайте вкратце сравним его с альтернативами..
...
У бабеля порядка 300 функций, методов и свойств. У TS там вообще какое-то запределное complexity, ещё и почти без документации.
...
Наконец, AST бабеля не является асмодостаточным, то есть мы не можем из него напрямую сгенерировать и результирующий скрипт и сорсмапы к нему - для этого надо окольными путями протягивать исходники. С сообщениями об ошибках - та же беда. У TS с этим, получше, но тут уже другая крайность - он вместе с бананом даёт вам и обезъяну, и джунгли, и даже свою солнечную систему и нескольких ипостасях.
...
Работает это просто: встретип незнакомый язык (определяемый по расширенияю файла), плагин сканирует воркспейс на предмет существования схемы для этого языка. Если найдёт несколько схем, то соединяет их.
...
На этом моё подвествование подходит к концу. Надеюсь мне удалось вас заинтересовать своими изысканиями. Тогда вам могут пригодиться следующие ссылки..
